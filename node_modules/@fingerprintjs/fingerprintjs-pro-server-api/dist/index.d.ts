...
interface components {
    schemas: {
         ...
         * @enum {string}
         */
        ErrorCode: 'RequestCannotBeParsed' | 'TokenRequired' | 'TokenNotFound' | 'SubscriptionNotActive' | 'WrongRegion' | 'FeatureNotEnabled' | 'RequestNotFound' | 'VisitorNotFound' | 'TooManyRequests' | '429 Too Many Requests' | 'StateNotReady' | 'Failed';
        Error: {
            /** @description Error code:
             *      * `RequestCannotBeParsed` - the query parameters or JSON payload contains some errors
             *               that prevented us from parsing it (wrong type/surpassed limits).
...
type ErrorPlainResponse = components['schemas']['ErrorPlainResponse'];
type ErrorResponse = components['schemas']['ErrorResponse'];
type SearchEventsFilter = paths['/events/search']['get']['parameters']['query'];
type SearchEventsResponse = paths['/events/search']['get']['responses']['200']['content']['application/json'];
/**
...
type EventsGetResponse = paths['/events/{request_id}']['get']['responses']['200']['content']['application/json'];
type RelatedVisitorsResponse = paths['/related-visitors']['get']['responses']['200']['content']['application/json'];
type RelatedVisitorsFilter = paths['/related-visitors']['get']['parameters']['query'];
/**
...
type EventsUpdateRequest = components['schemas']['EventsUpdateRequest'];
...
declare function getIntegrationInfo(): string;
/**
...
type PathParams<Path extends keyof paths> = ExtractPathParams<Path> extends never ? {
    pathParams?: never;
} : {
    pathParams: ExtractPathParams<Path> extends never ? never : string[];
};
type QueryParams<Path extends keyof paths, Method extends keyof paths[Path]> = ExtractQueryParams<paths[Path][Method]> extends never ? {
    queryParams?: any;
} : {
...
type GetRequestPathOptions<Path extends keyof paths, Method extends keyof paths[Path]> = {
    ...
    apiKey?: string;
    region: Region;
} & PathParams<Path> & QueryParams<Path, Method>;
/**
 ...
 *   parameters appended
 */
declare function getRequestPath<Path extends keyof paths, Method extends keyof paths[Path]>({ path, pathParams, apiKey, queryParams, region, method: _, }: GetRequestPathOptions<Path, Method>): string;

declare class FingerprintJsServerApiClient implements FingerprintApi {
     ...
     * ```
     * */
    getEvent(requestId: string): Promise<EventsGetResponse>;
    /**
     * Update an event with a given request ID
     * @description Change information in existing events specified by `requestId` or *flag suspicious events*.
     *
     ...
     * ```
     */
    updateEvent(body: EventsUpdateRequest, requestId: string): Promise<void>;
    /**
     * Delete data by visitor ID
     ...
     * ```
     */
    getVisits(visitorId: string, filter?: VisitorHistoryFilter): Promise<VisitorsResponse>;
    /**
     ...
     * ```
     */
    getRelatedVisitors(filter: RelatedVisitorsFilter): Promise<RelatedVisitorsResponse>;
    private getHeaders;
 ...
 * @private
 * */
declare function parseEventsResponse(unsealed: string): EventsGetResponse;
/**
 ...
 */
declare function unsealEventsResponse(sealedData: Buffer$1, decryptionKeys: DecryptionKey[]): Promise<EventsGetResponse>;
/**
 * @private
 * */
...
declare class RequestError<Code extends number = number, Body = unknown> extends SdkError {
    ...
    static fromPlainError(body: ErrorPlainResponse, response: Response): RequestError<number, {
        error: string;
    }>;
...
declare class TooManyRequestsError extends RequestError<429, ErrorResponse> {
    ...
    static fromPlain(error: ErrorPlainResponse, response: Response): TooManyRequestsError;
}

declare function getRetryAfter(response: Response): number;

export { AuthenticationMode, DecryptionAlgorithm, type DecryptionKey, type ErrorPlainResponse, type ErrorResponse, type EventsGetResponse, type EventsUpdateRequest, type ExtractQueryParams, type FingerprintApi, FingerprintJsServerApiClient, type IsValidWebhookSignatureParams, type Options, Region, type RelatedVisitorsFilter, type RelatedVisitorsResponse, RequestError, SdkError, type SearchEventsFilter, type SearchEventsResponse, TooManyRequestsError, UnsealAggregateError, UnsealError, type VisitorHistoryFilter, type VisitorsResponse, type Webhook, getIntegrationInfo, getRequestPath, getRetryAfter, isValidWebhookSignature, parseEventsResponse, unseal, unsealEventsResponse };
